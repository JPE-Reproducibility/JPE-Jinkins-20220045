function [thetas,thetab1,thetab2,Ap,U1,U2,V,mMb,Mbx,Vbx,sbx,u1,u2,Vs1x,Vs2x,Ms1,Ms2,Qbx,QS1,QS2,net_prof_b,cbs,Cs,Cs1,Cs2,net_prof_s1,net_prof_s2,tauxs1,tauxs2,profxs,pbs,profx] = ...
    solve_model_tariff(x,param_indx,param_state,param_fix)
...
for j=0:1:N2
    for i=0:1:N1
        proft((j)*(N1+1)+(i+1))=(gam^(-gam))*((gam-1)^(gam-1))*([i j]*([c1*(1+t_f1) c2*(1+t_f2)]'.^(1-alp)))^((1-gam)/(1-alp));
        % sales to be used later
        sales((j)*(N1+1)+(i+1))=((gam-1)/gam)^(gam-1)*([i j]*([c1*(1+t_f1) c2*(1+t_f2)]'.^(1-alp)))^((1-gam)/(1-alp));
        % construct price index, buyer in state state (i=n1,j=n2)
        pbs((j)*(N1+1)+(i+1)) = (gam/(gam-1))*([i j]*([c1*(1+t_f1) c2*(1+t_f2)]'.^(1-alp)))^(1/(1-alp));  
        cbs((j)*(N1+1)+(i+1)) = ([i j]*([c1*(1+t_f1) c2*(1+t_f2)]'.^(1-alp)))^(1/(1-alp));
        if i>0
    
              taus1_app((j)*(N1+1)+(i+1)) = kappa*kap_s*(c1*(1+t_f1))^(1-alp)*...
                                              ([i j]*([c1*(1+t_f1) c2*(1+t_f2)]'.^(1-alp)))^((1-gam)/(1-alp) -1);
        end 
      
        if j>0

              taus2_app((j)*(N1+1)+(i+1)) = kappa*kap_s*(c2*(1+t_f2))^(1-alp)*...
                                              ([i j]*([c1*(1+t_f1) c2*(1+t_f2)]'.^(1-alp)))^((1-gam)/(1-alp) -1);
        end 
    end
end
...
for j=0:1:N2 %# of type 2 seller
    ...
    for i=0:1:N1 %# of type 1 seller
        if i>0
            taus1_update((j)*(N1+1)+(i+1)) = sbarg/(1+sbarg*t_f1) * (proft((j)*(N1+1)+(i+1))-proft((j)*(N1+1)+(i))-j*(taus2((j)*(N1+1)+(i+1))-taus2((j)*(N1+1)+i))*(1+t_f2)...
               -(i-1)*((taus1((j)*(N1+1)+(i+1))-taus1((j)*(N1+1)+i)))*(1+t_f1));
        else
        ...
        if j>0
            taus2_update((j)*(N1+1)+(i+1)) = sbarg/(1+sbarg*t_f2) * (proft((j)*(N1+1)+(i+1))-proft((j-1)*(N1+1)+(i+1))-(j-1)*(taus2((j)*(N1+1)+(i+1))-taus2((j-1)*(N1+1)+(i+1)))*(1+t_f2)...
               -(i)*((taus1((j)*(N1+1)+(i+1))-taus1((j-1)*(N1+1)+(i+1))))*(1+t_f1));
        else
...
while norm(thetas_new-thetas)>1e-6||norm(thetab_new-thetab)>1e-6
    ...
    parfor b=1:Nx  
      ...
      % JT: adjust flow payoffs for price deflator and bargaining weights 
        tau1 = tauxs1{b}/Ap;  
        tau2 = tauxs2{b}/Ap;
        prof = profxs{b}/Ap - (s1.*tau1*(1+t_f1) + s2.*tau2*(1+t_f2));   
    ...
    %more general matching function
    scenario = 1; % Change this value to switch scenarios

    switch scenario
    case 1  % baseline
         X=V*(1-exp(-(U1+U2)/V));
    case 2  % crs Cobb-Douglas
         X=0.5*V*(U1+U2);
    end
