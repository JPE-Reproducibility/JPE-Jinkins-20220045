...
taus1 = zeros(Nstates, 1);
taus2 = zeros(Nstates, 1);

iter = 0; 
max_iter = 5000;
tol = 1e-10;

while norm(taus1 - taus1_app) > tol || norm(taus2 - taus2_app) > tol
    iter = iter + 1;
    if iter > max_iter
        warning('Max iterations exceeded in solving transfers');
        break;
    end

    taus1 = taus1 * 0.99 + taus1_app * 0.01;
    taus2 = taus2 * 0.99 + taus2_app * 0.01;

    for j = 0:N2
        for i = 0:N1
            idx = j * (N1 + 1) + (i + 1);
            if i > 0
                taus1_app(idx) = sbarg * (proft(idx) - proft(idx - 1) ...
                    - j * (taus2(idx) - taus2(idx - 1)) ...
                    - (i - 1) * (taus1(idx) - taus1(idx - 1)));
            end
            if j > 0
                taus2_app(idx) = sbarg * (proft(idx) - proft(idx - N1 - 1) ...
                    - (j - 1) * (taus2(idx) - taus2(idx - N1 - 1)) ...
                    - i * (taus1(idx) - taus1(idx - N1 - 1)));
            end
        end
    end
end
