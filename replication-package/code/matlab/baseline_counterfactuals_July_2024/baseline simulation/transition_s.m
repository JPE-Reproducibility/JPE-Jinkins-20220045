function [Ts1,Ts2] = transition_s(sb,thetab1,thetab2,param_indx,param_state,param_fix)
...
% This is the transition matrix for a specific relation from seller's perspective
 ...
 % Transition matrix Ts1 and Ts2
                %Define the diagonal
                Ts1 = diag(-sb.*thetaSum - (delta+delta_S)*nSum - delta_B);
                Ts2 = diag(-sb.*thetaSum - (delta+delta_S)*nSum - delta_B);
                
                %Define the four cases of adjacency cell
                %(s1-1,s2)
                Ts1(ms1ind2) = (delta+delta_S).*(s1(ms1ind) - 1);
                Ts2(ms1ind2) = (delta+delta_S).*s1(ms1ind);
                %(s1,s2-1)
                Ts1(ms2ind2) = (delta+delta_S).*s2(ms2ind);
                Ts2(ms2ind2) = (delta+delta_S).*(s2(ms2ind)-1);
                %(s1+1,s2)
        ...
        %Destruction of seller-match itself (can happen either match terminated, buyer exit, or seller exit
                ...
                Ts1(:,1) = delta+delta_B+delta_S;
                Ts2(:,1) = delta+delta_B+delta_S;
