...
%% 4.5 Data fraction of matches that die each year
data_match_death_frac = 0.774; %This is the number in the paper 
%below should divide by number of network connections, not number of firms
var_match_death_frac = data_match_death_frac * ( 1 - data_match_death_frac) / (8180 + 23562); 
%data_match_death_hazard = -log(1 - data_match_death_frac);
%var_match_death_haz = var_match_death_frac * 1 / (1 - data_match_death_frac)^2; %delta method
data_match_death_haz = data_match_death_frac;
var_match_death_haz = var_match_death_frac; 

data_match_death_reg_coef = -0.0119; %from Regression_pair_exit_tenure_rounded.csv in disclosed_data_moments folder.  Ignore lnNoSeller coefficient, as that is identically zero in our model.
var_match_death_reg_coef = (0.0004)^2; %from Regression_pair_exit_tenure_rounded.csv in disclosed_data_moments folder.  0.0004 is the standard error of the coefficient.

% %% Data life cycle moments
% data_life = readmatrix('05_avg_number_partners_by_yr_in_market_extended.xls', 'NumHeaderLines', 1);
% SPB_growth = data_life(2:10,8)';
% BPS_growth = data_life(2:10,9)';
% var_SPB_growth = data_life(2:10,10).^2;
% var_BPS_growth = data_life(2:10,11).^2;
% var_life_cycle = diag([var_SPB_growth;var_BPS_growth]);
...
% current calibration targets: NS/NB excluded, match-death regression included
data_moments = [cdf_SPB,cdf_BPS,PTran_mSPB,...
                 PTran_mBPS,data_moments_concen',data_interm_sh, data_match_death_reg_coef]';
Nmom = length(data_moments);
            
% consolidate blocks of covariance matrix for current moments
cov_v  = blkdiag(var_deg_cdf, PsiTrans_mSPB, PsiTrans_mBPS, var_concen, var_interm_sh, var_match_death_reg_coef); 

% Baseline ("old") moment vector and covariance, which include NS_NB and exclude match-death regression
baseline_data_moments = [cdf_SPB,cdf_BPS,PTran_mSPB,...
                 PTran_mBPS,data_moments_concen',data_interm_sh]';
baseline_cov_v  = blkdiag(var_deg_cdf, PsiTrans_mSPB, PsiTrans_mBPS, var_concen, var_interm_sh); 
