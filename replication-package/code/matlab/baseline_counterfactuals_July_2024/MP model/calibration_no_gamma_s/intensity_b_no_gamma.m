function Qb = intensity_b_no_gamma(sb,thetab1,thetab2,phi_1,phi_2,param_indx,param_state,param_fix,lambda)
...
delta = param_fix{12};
%lambda = param_fix{22}; % shock to profit function
...
%Intensity matrix Qb
    Qb = diag(-sb.*thetaSum - (delta+delta_S)*nSum - lambda*phi_1*s1 - lambda*phi_2*s2 - delta_B);
    % filling in the off-diagonal elements            
    Qb(ms1ind2) = (delta+delta_S+lambda*phi_1).*s1(ms1ind); %sk - 1, k = 1 JT: lose a low quality seller
    Qb(ms2ind2) = (delta+delta_S+lambda*phi_2).*s2(ms2ind); %sk - 1, k = 2 JT: lose a high quality seller
