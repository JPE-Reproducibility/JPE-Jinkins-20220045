function Qb = intensity_b(sb,thetab1,thetab2,param,param_indx,param_state,param_fix)
...
%Intensity matrix Qb
    Qb = diag(-sb.*thetaSum - (delta1+delta_S)*n1Sum - (delta2+delta_S)*n2Sum - delta_B);
    % filling in the off-diagonal elements            
    Qb(ms1ind2) = (delta1+delta_S).*s1(ms1ind); %sk - 1, k = 1 JT: lose a low quality seller
    Qb(ms2ind2) = (delta2+delta_S).*s2(ms2ind); %sk - 1, k = 2 JT: lose a high quality seller
