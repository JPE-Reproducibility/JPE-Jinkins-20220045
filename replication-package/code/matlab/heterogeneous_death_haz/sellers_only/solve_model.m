function [thetas,thetab,Ap,U1,U2,V,mMb,msb,u1,u2,Ms1,Ms2,Qbx,QS1,QS2,net_prof_b,Cs,Cs1,Cs2,net_prof_s1,net_prof_s2,avg_match_death_haz,match_death_reg_coef,delta2] = ...
...
Nx = param_fix{4};
alp = param_fix{7};
% gam   = param_fix{8}; % now treated as estimated parameter--see line 34
% kappa = param_fix{9};
wt_old = param_fix{13}; 
...
%seller death hazards
delta1 = param{13};     % type 1 seller match death haz
delta2 = param{14};     % type 2 seller match death haz
%types of buyers
var_mu = param{9};
...
for j=0:1:N2
    for i=0:1:N1
        ...
        % JT: comment in define_parameters says kappa isn't used anymore                                  
...
while norm(thetas_new-thetas)>1e-6||norm(thetab_new-thetab)>1e-6
    ...
    parfor b=1:Nx  
      ...
      % JT: adjust flow payoffs for price deflator, edied by DX to take out bargaining weights 
        ...
        [Vs1,Vs2] = val_s(tau1,tau2,sb,thetab1,thetab2,param,param_indx,param_state,param_fix);
    ...
    QS1 = intensity_s(u1,thetas,param_fix,delta1);
    QS2 = intensity_s(u2,thetas,param_fix,delta2);
    ...
    %% set match death hazard of seller type 2 to hit average match death hazard
    
    % buyer type dist in rows, one row for each state
    w = repmat(wt_x, (N1+1)*(N2+1), 1); 

    % s1 is the number of low type suppliers by state
    % mMb is the distribution (in columns) by type (row) 
    % Since the total mass of buyers is 1, S1 is the mass of 
    % low type suppliers.
    S1 = sum(sum( repmat(s1, 1, Nx) .* mMb .* w ));
    S2 = sum(sum( repmat(s2, 1, Nx) .* mMb .* w ));
    S = S1 + S2; %Total active supplier mass

    % Set the target weighted hazard (excluding delta_S and delta_B)
    target = 0.774 - (delta_S + delta_B);

    % Solve for delta2
    delta2 = max((target * S - delta1 * S1) / S2, 1e-12);
    param{14} = delta2;
    avg_match_death_haz = (delta1 * S1 + delta2 * S2) / S + delta_S + delta_B;

    match_death_reg_coef = match_death_reg(N,Ms1,Ms2,delta1,delta2,w1_x,w2_x,delta_S,delta_B);
...
%catch
%         disp('WARNING: Error in solve model')    
