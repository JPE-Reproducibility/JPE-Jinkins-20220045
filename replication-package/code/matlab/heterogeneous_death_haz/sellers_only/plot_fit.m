...
moment_compare_mTS = [model_mTB, PTran_mSPB']; % sellers per buyer (JT: corrected 7-20-24)
moment_compare_mTB = [model_mTS, PTran_mBPS']; % buyers per seller (JT: corrected 7-20-24)
...
xlabel('model');
ylabel('data');
title('variable costs over retail sales')
hold off
...
scatter(z,z,mkr)
hold on
scatter(avg_match_death_haz,data_match_death_frac);
xlabel('model');
ylabel('data');
title('average fraction of annual match separation')
hold off

subplot(2,1,2)
scatter(z,z,mkr)
hold on
scatter(match_death_reg_coef,data_match_death_reg_coef);
xlabel('model');
ylabel('data');
title('regression coefficient, match separation prob on current match number')
hold off

save results/plotfit_SellerHetHaz
